...
def update_user(event):
    try:
        ...
        # Check for approval status updates in the request body
        is_approval_update = ('UserAccess' in body or 'ApprovalStatus' in body)
        logger.info(f"Approval update detected: {is_approval_update}, Body: {json.dumps(body)}")
        
        if is_approval_update:
            # Check if we have DynamoDB table configured
            if not USER_TABLE:
                logger.warning("USER_TABLE environment variable not set, skipping DynamoDB update")
                return generate_response(200, {'message': 'User updated in Cognito only. DynamoDB not configured'})
            
            # Set values based on the request
            ...
            update_expression_parts = ["UpdatedAt = :updated"]
            expression_attr_values = {":updated": current_time}
            
            # Add UserAccess to update if present
            if user_access is not None:
                update_expression_parts.append("UserAccess = :access")
                expression_attr_values[":access"] = user_access
            
            # Add ApprovalStatus to update if present
            if approval_status:
                update_expression_parts.append("ApprovalStatus = :status")
                expression_attr_values[":status"] = approval_status
            
            # Add Status to update if present (using expression attribute names since Status is reserved word)
            if status:
                update_expression_parts.append("#user_status = :user_status")
                expression_attr_values[":user_status"] = status
                expression_attr_names = {"#user_status": "Status"}
            else:
            ...
            # Log the DynamoDB update parameters for debugging
            logger.info(f"DynamoDB update parameters: Table={USER_TABLE}, Key={user_id}")
            ...
            try:
                ...
                response = user_table.update_item(**update_params)
                
                # Log the response for verification
                logger.info(f"DynamoDB update successful: {json.dumps(response.get('Attributes', {}))}")
                
                # Double-check the update by getting the latest record
                get_response = user_table.get_item(Key={'UserId': user_id})
                if 'Item' in get_response:
                    logger.info(f"User record after update: UserAccess={get_response['Item'].get('UserAccess')}, ApprovalStatus={get_response['Item'].get('ApprovalStatus')}")
                
            except Exception as dynamo_error:
                # Still return success for the Cognito update even if DynamoDB fails
                logger.error(f"Error updating user in DynamoDB: {str(dynamo_error)}")
                return generate_response(200, {
                    'message': 'User updated in Cognito but DynamoDB update failed', 
