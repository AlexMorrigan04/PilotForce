...
def extract_email_domain(email):
    ...
    # Properly extract domain after the @ symbol
...
def get_company_by_domain(email_domain):
    ...
    try:
        ...
        # As a fallback, scan the table if the GSI query didn't work
        try:
            ...
            if 'Items' in scan_response and scan_response['Items']:
                ...
                # If found via scan but index query failed, there might be an issue with the GSI
        ...
        # If still not found, check if it might be capitalization issue by doing a scan with contains
        try:
            ...
            # Simple local filtering since DynamoDB doesn't support case-insensitive searches natively
            ...
            if matches:
                ...
                # Update the company record to use the normalized domain for future lookups
                try:
...
def create_company(email_domain, company_name=None):
    ...
    # Double-check if company exists before creating a new one
    ...
    try:
        ...
        # Create the company object with normalized domain
        company = {
            'CompanyId': company_id,
            'EmailDomain': normalized_domain,  # Store domain in lowercase
...
def get_or_create_company_for_user(email):
    ...
    try:
        # First check if company exists - always use lowercase for lookups
        companies_table = dynamodb.Table(COMPANIES_TABLE)
        
        # Query for companies with this domain
        ...
        # Handle case where multiple companies exist with the same domain
        if len(companies) > 1:
        ...
        # Fallback to direct creation in case of errors
        try:
            company_name = get_company_name_from_domain(email_domain)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Extract signup data with proper debugging
        username = body.get('username', '')
        password = body.get('password', '')
        
        # Extract attributes from the proper location
        attributes = body.get('attributes', {})
        
        # Get specific attributes with fallbacks
