...
export const getTokenInfo = (token: string | null): {
  ...
  // Special handling for test tokens generated by Lambda in test mode
  if (token.startsWith('test-id-token') || token.startsWith('test-access-token')) {
    // Generate a future expiration time (1 hour from now)
    ...
    return {
      ...
      expiresAt,
      isExpired: false,
      timeToExpiry: 60 * 60 * 1000, // 1 hour in milliseconds
      payload: { 
        sub: 'test-user',
