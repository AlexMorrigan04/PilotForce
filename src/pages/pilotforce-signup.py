...
def disable_user_account(username, user_pool_id):
    """
    Disable a user account in Cognito to require administrator approval
    """
    try:
        # Disable user account until admin approval using built-in Cognito status
        cognito.admin_disable_user(
            UserPoolId=user_pool_id,
            Username=username
        )
        print(f'Disabled user account {username} until admin approval')
        return True
    except Exception as e:
        print(f'Error disabling user account: {str(e)}')
        return False

def check_email_exists(email):
    """
    Check if the email already exists in Cognito or DynamoDB
    Returns: (exists, source, message)
    - exists: boolean indicating if email exists
    - source: 'cognito' or 'dynamodb' or None
    ...
    try:
        ...
        if user_pool_id:
            # Try to find user by email filter
            ...
            if response.get('Users', []):
                print(f"Email '{email}' already exists in Cognito")
                return True, 'cognito', f"An account with email '{email}' already exists."
    except Exception as e:
        print(f"Error checking Cognito for email: {str(e)}")
    
    # Then check DynamoDB
    try:
        users_table = dynamodb.Table(USERS_TABLE)
        
        # First try with a query on the EmailIndex
        try:
            ...
            if response.get('Items', []):
                print(f"Email '{email}' already exists in DynamoDB (via index)")
                return True, 'dynamodb', f"An account with email '{email}' already exists."
        except Exception as query_error:
            print(f"Error querying EmailIndex: {str(query_error)}")
            
            # Fallback to scan if the query fails
            scan_response = users_table.scan(
                FilterExpression=Attr('Email').eq(email)
            )
            if scan_response.get('Items', []):
                print(f"Email '{email}' already exists in DynamoDB (via scan)")
                return True, 'dynamodb', f"An account with email '{email}' already exists."
    except Exception as e:
        print(f"Error checking DynamoDB for email: {str(e)}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # NEW CODE: Check if email already exists before proceeding
        email_exists, source, error_message = check_email_exists(email)
        if email_exists:
            print(f"Email '{email}' already exists in {source}")
            return {
                'statusCode': 400,
                'headers': headers,
                'body': json.dumps({
        ...
        # Determine proper user role based on email domain existence BEFORE creating user attributes
        companies_table = dynamodb.Table(COMPANIES_TABLE)
        if email:
            email_domain = extract_email_domain(email)
            print(f"Determining user role based on email domain: {email_domain}")
            
            # Check if this email domain exists in the Companies table
            domain_exists = False
            if email_domain:
                try:
                    # Check for existing companies with the NORMALIZED email domain
                    normalized_domain = email_domain.lower()
                    
                    print(f"Checking for domain: '{normalized_domain}'")
                    
                    # Use scan instead of query for more reliable results
                    scan_response = companies_table.scan()
                    all_companies = scan_response.get('Items', [])
                    
                    # Debug - print all companies and their domains
                    for company in all_companies:
                        print(f"Company: {company.get('CompanyId')}, Domain: {company.get('EmailDomain')}")
                    
                    # Check if ANY company has this domain - using case-insensitive comparison
                    for company in all_companies:
                        company_domain = company.get('EmailDomain', '').lower()
                        if company_domain == normalized_domain:
                            domain_exists = True
                            print(f"Found existing company with domain '{normalized_domain}': {company.get('CompanyId')}")
                            break
                    
                    if not domain_exists:
                        print(f"No company found with domain '{normalized_domain}'")
                except Exception as e:
                    print(f"Error checking domain existence: {str(e)}")
            
            # Set user role based on domain existence
            if not domain_exists:
                print(f"No record of domain '{email_domain}' found - assigning CompanyAdmin role")
                user_role = 'CompanyAdmin'
            else:
                print(f"Domain '{email_domain}' exists - assigning User role")
                user_role = 'User'
        
        print(f"Final user role determined: {user_role}")
        
        # Now create the user attributes
        user_attributes = []
        ...
        try:
            ...
            try:
                ...
                user_item = {
                    ...
                    'Status': 'CONFIRMED',  # The record exists in DynamoDB
                    'UserAccess': False,  # But they don't have access yet
                    'ApprovalStatus': 'PENDING'  # Add a specific approval status field
                }
            ...
            if user_pool_id:
                try:
                    ...
                    try:
                        ...
                        # Disable user account until admin approval using built-in Cognito status
                        disable_user_account(username, user_pool_id)
                    except Exception as attr_error:
                        print(f'Could not set user attributes or disable user: {str(attr_error)}')
                    
                    print(f'User {username} confirmed but account disabled pending approval')
                    
                    # Check if this is a new domain (company) or existing one
                    email_domain = extract_email_domain(email)
                    is_new_domain = True
                    found_company_name = ''
                    ...
                    if email_domain:
                        ...
                        if company:
                            is_new_domain = False
                            found_company_name = company.get('CompanyName', get_company_name_from_domain(email_domain))
                    
                    return {
                        'statusCode': 200,
                        'headers': headers,
                        'body': json.dumps({
                            'message': 'User created and confirmed successfully but requires admin approval',
                            'userId': user_sub,
                            ...
                            'companyName': found_company_name or company_name
                        })
                    }
